...
export class AsistenciaController {
  ...
  async crearAsistencia(@Body() createAsistenciaDto: CreateAsistenciaDto, @Req() req: Request) {
    ...
    const crearAsistencia = await this.asistenciaService.crearAsistencia(
      createAsistenciaDto.id_pertenece,
      usuario.Id_usuario_pk,
      asistencia[0]
    );
    req["logMessage"] =
      `Registro asistencia en la fecha ${asistencia[0].fecha} con ${asistencia[0].horas} horas para el alumno ${createAsistenciaDto.id_pertenece}`;
  ...
  @Get("traer")
  async traerTodas(@Req() req: Request, @Query("alumno") id_pertenece?: string) {
    const asistencias = await this.asistenciaService.traerTodasEncargado(req["Usuario"].Id_usuario_pk);
    ...
    if (id_pertenece) {
      ...
      return asistencias.filter(asistencia => {
        return asistencia.id_pertenece === idPerteneceNum;
      });
    }
    return asistencias;
  }
  ...
  @Put("actualizar")
  async actualizarPorEncargado(@Req() req: Request, @Body() updateAsistenciaDto: UpdateAsistenciaDto) {
  ...
  @Get("horas")
  async obtenerHoras(@Req() req: Request, @Query("alumno", ParseIntPipe) id_pertenece: number) {
